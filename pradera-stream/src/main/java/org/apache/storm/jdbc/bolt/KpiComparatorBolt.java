...
public class KpiComparatorBolt extends JdbcLookupBolt{
	...
	protected void process(Tuple tuple) {
		...
		try {
			...
			try {
				comparatorMap	  =	(Map) comparatorsMap.get(_streamId);
				Validate.notNull(comparatorMap, "The comparatorMap  must not be null");
			} catch (Exception e) {
				if ( e instanceof NullPointerException) {
					LOG.error(" NO EXISTE EL VALOR MAPEADO DEL streamId  : "+ _streamId + " EN EL COMPONENTE : KpiComparatorBolt");
					throw e;
				}
			}
